...
function Key(props) {
  const [state, setState] = useState("pr-7 bg-gray-400 hover:bg-gray-450 active:pr-7 dark:bg-zinc-500 dark:text-white dark:hover:bg-zinc-600");
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      if (props.state === "N") setState("bg-wrong text-white dark:bg-wrong");
    }, 350);
  }, [props.state]);
  ...
  return (
    <button
      className={
        x +
        state +
        " h-13 grid items-center rounded font-semibold cursor-pointer"
      }
      onClick={returnKey}
    >
